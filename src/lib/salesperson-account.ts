...
/**
 * Çoklu cihaz modunu kapatır (kullanıcı adı/şifreyi temizler). Var olan
 * cihazlar dokunulmadan kalır - admin ayrıca isActive'i kapatarak erişimi
 * tamamen kesebilir (bkz. device-lock.ts getAuthorizedDevice).
 */
export async function clearSalespersonCredentials(salespersonId: string) {
  ...
  if (!existing.username) {
    throw new Error("Bu plasiyerde zaten kullanıcı adı/şifre yok");
  }

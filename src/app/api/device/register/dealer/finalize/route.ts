...
import { finalizeApprovedDealerRequest } from "@/lib/device-access";
...
export async function POST() {
  try {
    ...
    const { device, dealerName } = await finalizeApprovedDealerRequest(requestToken);
    ...
    response.cookies.set(DEVICE_ACTOR_NAME_COOKIE, dealerName, opts);
    ...
    console.error("POST /api/device/register/dealer/finalize failed:", err);
    const message =
      err instanceof Error ? err.message : "Bayi girişi tamamlanamadı";
